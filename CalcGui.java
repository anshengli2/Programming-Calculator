...
public class CalcGui extends JPanel implements ActionListener, MouseListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			...
			if (e.getSource() == equalsButton) {
				...
				bitCounter++;
				if (bitCounter == 2) {
					bitButton.setText("DWORD");
					c.setQword(false);
					c.setDword(true);
					c.setWord(false);
					c.setByte(false);
					c.setInput(String.valueOf(c.rollOver(Long.parseLong(c.getInput()))));
					inputField.setText(c.getInput());
					updateConversion(c.getInput());
				} else if (bitCounter == 3) {
					bitButton.setText("WORD");
					c.setQword(false);
					c.setDword(false);
					c.setWord(true);
					c.setByte(false);
					...
					inputField.setText(c.getInput());
					updateConversion(c.getInput());
				} else if (bitCounter == 4) {
					bitButton.setText("BYTE");
					c.setQword(false);
					c.setDword(false);
					c.setWord(false);
					c.setByte(true);
	...
	public void switching(int val) {
		...
		changeModes(bitButton, null, 0, val);
		if (val == 0) {
			randomCount = 1;
		} else {
			randomCount = 0;
	...
	public void checkInput(JButton b[], int i) {
		if (c.getByte()) {
			byte max = Byte.MAX_VALUE;
			byte min = Byte.MIN_VALUE;
			String tempInput = "";
			if (Long.parseLong(c.getInput()) > max || Long.parseLong(c.getInput()) < min) {
				tempInput = c.getInput().substring(0, c.getInput().length() - 1);
				c.setInput("");
				c.storeInput(tempInput);
			}
		}
		else if(c.getDword()) {
...
//		}
	...
	public void mouseEntered(MouseEvent e) {
		...
		if (e.getSource() == secButton) {
			...
			bitButton.setBorder(BorderFactory.createLineBorder(c, 3));
		}
	}
	...
	public void mouseExited(MouseEvent e) {
		...
		if (e.getSource() == secButton) {
			...
			bitButton.setBorder(BorderFactory.createLineBorder(c));
		}
	}
